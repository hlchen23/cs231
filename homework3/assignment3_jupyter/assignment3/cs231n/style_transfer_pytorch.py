...
def content_loss(content_weight, content_current, content_original):
    ...
    Lc = content_weight * ((content_current-content_original)**2).sum()
    return Lc
...
def gram_matrix(features, normalize=True):
    ...
    x = features.view(N,C,H*W) # N, C, M
    x_t = x.permute(0,2,1) # N, M, C 实现特定维度上的转置
    gram = torch.matmul(x,x_t)
    if normalize:
          gram = gram / (C*H*W)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx,i in enumerate(style_layers):
          ...
          style_loss += style_weights[idx]*((gram-style_targets[idx])**2).sum()
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    y1 = img[:,:,:,1:W]
    y2 = img[:,:,:,0:(W-1)]
    loss = ((((x1-x2)**2)).sum()+(((y1-y2)**2)).sum()) * tv_weight
