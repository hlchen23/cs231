...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        H_conv = 1 + input_dim[1] + 2 * ((filter_size-1)//2) - filter_size
        W_conv = 1 + input_dim[2] + 2 * ((filter_size-1)//2) - filter_size
        H_pool = 1 + (H_conv - 2) // 2
        W_pool = 1 + (W_conv - 2) // 2
        W2_dim = num_filters * H_pool * W_pool
    ...
    def loss(self, X, y=None):
        ...
        dW2 += (2*0.5)*self.reg*W2
        dW3 += (2*0.5)*self.reg*W3
        grads["W1"] = dW1
        grads["W2"] = dW2
        grads["W3"] = dW3
        grads["b1"] = db1
