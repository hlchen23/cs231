...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(iters):
        ...
        print('pre_class',score.argmax(axis=1))
        print('y',y)
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    model.zero_grad()
    score = model(img)
    target_score = score[:,target_y] # 让指定目标对应的分数梯度上升
    target_score.backward()
    dimg = img.grad + 2*l2_reg*img# 只需要在导数里面体现正则化项就可以，原损失不必体现
    with torch.no_grad():# 进行梯度下降的过程本身不需要求导
        img += learning_rate*dimg/torch.norm(dimg) #梯度上升，＋ 
