...
def generator_loss(logits_fake):
    ...
    fake_loss = bce_loss(logits_fake, fake_labels) # 以假乱真程度越高，loss越小
    loss = fake_loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 1/2 * (scores_real-real_labels)**2 + 1/2  * (scores_fake-fake_labels)**2
    loss = loss.mean()
...
def ls_generator_loss(scores_fake):
    ...
    fake_labels_cheat = torch.ones_like(scores_fake).type(dtype)
    loss = 1/2 * (scores_fake-fake_labels_cheat)**2
    loss = loss.mean()
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        # N * 1 * 28 * 28
        nn.Conv2d(1,32,kernel_size=5,stride=1),# 黑白二值单通道，所以是1,32个卷积核，所以输出是32层
        nn.LeakyReLU(0.01),
